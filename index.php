...
// ---- PRG: GET時にセッションから値を復元 ----
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    $_POST = [];
    $resultText = '';
    $errorMessage = '';

    if (isset($_SESSION['last_resultText'])) {
        $resultText = $_SESSION['last_resultText'];
    }
    if (isset($_SESSION['last_postData'])) {
        $_POST = $_SESSION['last_postData'];
    }
    if (isset($_SESSION['last_errorMessage'])) {
        $errorMessage = $_SESSION['last_errorMessage'];
    }
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (!isset($_SERVER['CONTENT_TYPE']) || strpos($_SERVER['CONTENT_TYPE'], 'application/x-www-form-urlencoded') === false) {
        ...
        if ($textError) {
            ...
            if ($curlError) {
                ...
                if ($httpCode === 429) {
                    // 追加: 使用制限時のエラー
                    $errorMessage = "APIの使用制限に達しました。時間をおいて再度お試しください。";
    ...
    // ---- PRG用にセッションへ保存 ----
    $_SESSION['last_resultText'] = $resultText;
    $_SESSION['last_errorMessage'] = $errorMessage;
    $_SESSION['last_postData'] = $_POST;
                    ...
                    <?php foreach ($allowedStyles as $styleOption): ?>
                        <label class="radio-item">
                            <input type="radio" name="style" value="<?= htmlspecialchars($styleOption, ENT_QUOTES, 'UTF-8') ?>"
                                <?= (($_POST["style"] ?? '') === $styleOption) ? 'checked' : '' ?>>
                            <?= htmlspecialchars($styleOption, ENT_QUOTES, 'UTF-8') ?>
                        </label>
                    <?php endforeach; ?>
                </div>
            </div>
